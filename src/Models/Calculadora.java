...
public class Calculadora {
    ...
    private static final int Suma = 10;  // +       
    private static final int Resta = 20; // -   
    private static final int Mult = 30;   // *
    private static final int Div = 40;    // /
    private static final int Expo = 50;    // ^
    private static final int FUN = 60;    // 
    private static final int PI = 70;    // (
    private static final int PD = 80;    // )
    ...
    public boolean iniEval(){
        ...
        if(E(refV, refPostFijo)){ 
            ...
            if(token == 0){ // Fin de cadena
                ...
                this.PostF = refPostFijo.valor.trim(); // Limpiar espacios al inicio/final
                return true;
            }
        }
        return false;
    ...
    public boolean Ep(EnvoltorioFloat v, EnvoltorioString Post){
        ...
        token = L.yylex();
        if(token == Suma || token == Resta){ // + o -
            if(T(refV2, refPost2)){
                // Cálculo de valor
                v.flotante = v.flotante + (token == Suma ? refV2.flotante : -refV2.flotante);
                
                // Generación de Postfijo
                Post.valor = Post.valor + " " + refPost2.valor + " " + (token == Suma ? "+" : "-");
    ...
    public boolean Tp(EnvoltorioFloat v, EnvoltorioString Post){
        ...
        token = L.yylex();
        if(token == Mult || token == Div){ // * o /
            if(P(refV2, refPost2)){
                // Cálculo de valor
                v.flotante = v.flotante * (token == Mult ? refV2.flotante : 1/refV2.flotante);
                
                // Generación de Postfijo
                Post.valor = Post.valor + " " + refPost2.valor + " " + (token == Mult ? "*" : "/");
